...
module.exports = class Spinners {
   ...
   * @param {string} preSpace - string that is between preText and text
   * @param {string} postSpace - string that is between postText and text
   * @returns {undefined}
   */
  constructor(spinners, preText, postText, preSpace, postSpace) {
    // bind props
    this.preText = preText
    this.postText = postText
    this.preSpace = preSpace ? preSpace : preText.length ? ' ' : ''
    this.postSpace = postSpace ? postSpace : postText.length ? ' ' : ''
   ...
   */
  spinnerText(text, opts) {
    return [
      opts && opts.preText ? opts.preText : this.preText,
      opts && opts.preSpace ? opts.preSpace : this.preSpace,
      text,
      opts && opts.postSpace ? opts.postSpace : this.postSpace,
      opts && opts.postText ? opts.postText : this.postText
    ].join('')
  }
